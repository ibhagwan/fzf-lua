...
local convert_diagnostic_type = function(severity)
  ...
  if type(severity) == "string" and not tonumber(severity) then
    ...
    return vim.diagnostic and vim.diagnostic.severity[severity:upper()]
      or vim.lsp.protocol.DiagnosticSeverity[severity:gsub("^%l", string.upper)]
  else
...
M.diagnostics = function(opts)
  ...
  for k, v in pairs(signs) do
    ...
    if utils.__HAS_NVIM_010 then
      ...
      if type(sign_confs) ~= "table" or utils.tbl_isempty(sign_confs) then
        sign_confs = nil
      end
      opts.__signs[v.severity].text = (
        not opts.diag_icons
        or not sign_confs
        or not sign_confs.text
        or not sign_confs.text[level]
      )
          and v.default
        or vim.trim(sign_confs.text[level])
      ...
      if utils.tbl_isempty(sign_def) then
        sign_def = nil
      end
      opts.__signs[v.severity].text = (not opts.diag_icons or not sign_def or not sign_def[1].text)
          and v.default
        or vim.trim(sign_def[1].text)
  ...
  local diag_results = vim.diagnostic
      and vim.diagnostic.get(not opts.diag_all and curbuf or nil, diag_opts)
    or opts.diag_all and vim.lsp.diagnostic.get_all()
    or { [curbuf] = vim.lsp.diagnostic.get(curbuf, opts.client_id) }
          ...
          if not utils.tbl_isempty(diag) and filter_diag_severity(opts, diag.severity) then
            ...
            vim.schedule(function()
              ...
              if not entry then
                ...
                entry = string.format(
                  "%s%s%s",
                  icon and string.format("%s%s%s", icon, opts.icon_padding or "", utils.nbsp) or "",
                  opts.diag_source
                      and utils.ansi_from_hl(
                        opts.color_headings and sign_def.texthl,
                        string.format(
                          "%s%s%s%s",
