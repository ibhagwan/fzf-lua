...
local get_files_cmd = function(opts)
  ...
  elseif utils.__IS_WINDOWS then
    ...
    command = string.format("find %s %s", search_paths and search_paths or ".", opts.find_opts)
  end
  for k, v in pairs({
...
M.zoxide = function(opts)
  ...
  opts.preview = (function()
    ...
    return vim.fn.executable("lsd") == 1
      ...
      or vim.fn.executable("eza") == 1 and "eza -la --color=always --icons -g --group-directories-first {2}"
      or "ls -la {2}"
  end)()
