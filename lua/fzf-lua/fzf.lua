  ...
  if contents then
    if type(contents) == "string" and #contents > 0 then
      ...
      if not fish_shell or bin_is_sk then
        table.insert(cmd, "<")
        table.insert(cmd, vim.fn.fnameescape(fifotmpname))
      else
        FZF_DEFAULT_COMMAND = string.format("cat %s", vim.fn.fnameescape(fifotmpname))
      end
    end
  end

  table.insert(cmd, ">")
  table.insert(cmd, vim.fn.fnameescape(outputtmpname))
