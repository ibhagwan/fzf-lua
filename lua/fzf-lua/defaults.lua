...
function M._preview_pager_fn()
  return vim.fn.executable("delta") == 1 and ("delta --width=$COLUMNS --%s"):format(vim.o.bg) or nil
end
...
M.defaults = {
  ...
  previewers = {
    ...
    bat = {
      ...
      cmd = function()
        return vim.fn.executable("batcat") == 1 and "batcat" or "bat"
      end,
    ...
    git_diff = {
      ...
      -- cmd_modified  = { "git", "diff", "--color", "HEAD" },
      -- cmd_untracked = { "git", "diff", "--color", "--no-index", "/dev/null" },
      _fn_git_icons = function()
        return M.globals.git.icons
      end,
    ...
    man = {
      ...
      cmd = function()
        return M._man_cmd_fn()
      end,
    },
    man_native = {
      _ctor = previewers.fzf.man_pages,
      cmd = function()
        return M._man_cmd_fn(true)
      end,
    },
    help_tags = {
...
M.defaults.files = {
  ...
  _actions = function()
    return M.globals.actions.files
  end,
