...
---@param bufnr integer
---@param zindex integer
---@param cancel fun(): boolean
---@param opts TSContext.UserConfig
function M.schedule_update(winid, bufnr, zindex, cancel, opts)
  ...
  opts = vim.tbl_extend("force", opts or {}, { multiwindow = true })
  for _, t in ipairs({ 0, 20, 50, 100 }) do
    vim.defer_fn(function()
      if context_updated or cancel() or not parser:is_valid(true) then return end
      context_updated = true
      M.update(winid, bufnr, opts)
    end, t)
  end
end
